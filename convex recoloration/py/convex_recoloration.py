...
for file in files:
    ...
    # obtendo a lista de variaveis e respectivos coeficientes para função obj.
    lista_variaveis = df['nome_variavel']
    lista_coeficientes = df['coeff']

    # CRIANDO AS VARIÁVEIS DO MODELO ------------------------------------------
    variables = adicionar_variaveis_modelo(m, lista_variaveis, "modelo_var")

    # ADICIONANDO EXPRESSÃO LINEAR DA FUNÇÃO OBJETIVO -------------------------
    m = criar_funcao_objetivo(m, lista_variaveis, variables, lista_coeficientes)
    ...
    for numero_vertice in map(int, obter_lista_vertices(path)):
        ...
        # somados em uma restrição linear para garantir que assumam apenas uma
    ...
    for cor_i in cores:
        ...
        v_combinations = v_combinations +\
                         list(itertools.combinations([k for k in lista_variaveis
                                                      if 'cor'+str(cor_i) in k], 3))
    ...
    gerando_relatorio_otimizacao(diretorio_resultados,
                                 file,
                                 m,
                                 obter_numero_vertices(diretorio + file),
                                 obter_numero_cores(diretorio + file),
                                 obter_lista_vertices_cor(diretorio + file).to_string(index=False)
                                 )
